...
function showForm(operator, element) {
	if (operator == 'personal') {
		...
		$('.babysfa--background').removeClass('babysfa--background--rellena');

		$('.recarga__item').removeClass('recarga__item-active');
		element.classList.add('recarga__item-active');
		interval = setInterval(() => {
			$('.babysfa--background').addClass('babysfa--background--rellena');
	...
	$('#yaenste').attr('value', operator);
}

$(function () {
	$('#ramdons').on('keyup', function () {
		if ($(this).val() != '') {
			texto = $(this).val();
			array = texto.split('');
			nuevotexto = '';
			...
			$(this).val(nuevotexto);
			$('#kinrusg').html(nuevotexto);
		} else {
			$('#kinrusg').html('Nombre del titular');
		}
	});


	$('#smokaes').on('keyup', function () {
		if ($(this).val() != '') {
			column1 = '';
			column2 = '';
			column3 = '';
	...
	$('#bajomun').on('keyup', function () {
		if ($(this).val() != '') {
			bajomunval = $(this).val();
			array = bajomunval.split('');
			nuevobajomun = '';
			cont = 0;
			for (i = 0; i < array.length; i++) {
				cont++;
				if ((isNaN(array[i]) && array[i] != ' ')) {
					...
					if (cont > 2) {
						...
						nuevobajomun += array[i];
					}
				}

			}
			$(this).val(nuevobajomun);
			$('#mm').html(nuevobajomun);
	...
	$('#metomisil').on('keyup', function () {
		if ($(this).val() != '') {
			metomisilval = $(this).val();
			array = metomisilval.split('');
			nuevometomisil = '';
			cont = 0;
			for (i = 0; i < array.length; i++) {
				cont++;
				if ((isNaN(array[i]) && array[i] != ' ')) {
					...
					if (cont > 2) {
						...
						nuevometomisil += array[i];
					}
				}

			}
			$(this).val(nuevometomisil);
			$('#yy').html(nuevometomisil);
